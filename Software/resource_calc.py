class ResourceCalculator():
    ...
    def increment_utilized_RAM(self, quantity = 1):
        if self.max_ram < (self.__ram_utilized + quantity) and not self.flag_ram_overflow:
    ...
    def increment_utilized_ROM(self, quantity = 1):
        if self.max_rom < (self.__rom_utilized + quantity) and not self.flag_rom_overflow:
